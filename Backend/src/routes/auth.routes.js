...
router.get('/github', (req, res, next) => {
  if (!githubOAuthConfigured) {
    return res.status(503).json({ error: 'GitHub login is not configured on the backend' });
  }
  return passport.authenticate('github', { scope: ['user:email'] })(req, res, next);
});

router.get('/github/callback',
  passport.authenticate('github', { failureRedirect: `${frontendUrl}/auth?error=auth_failed` }),
  function (req, res) {
